...
function createProfileCard(news) {
    var news_id = news.news_id;
    var short_description = news.short_description;
    var front_image_path = news.front_image_path;
    var title = (news.title !=null ? (news.title + ':') : '');
...
function createExpandedCard(news) {
    ...
    var title = (news.title !=null ? (news.title + ':') : '');
    var url = `/mobile/each-news/${news_id}`;
    var url1 = `/mobile/profile/${news.user_id}`;
    if(front_image_path != null) {
        if(front_image_path.slice(0,5) == "https") {
            var img = `<img src="${front_image_path}" alt="" style="border-radius: 5px">`;
        } else {
...
function share(news_id,title) {
    ...
    if(url[0] == 'http:'){
        ...
        const shareData = {
            title: title,
            text: 'न्यूज़ के लिए आज ही ऐप इंस्टॉल करें।%0Dhttps://play.google.com/store/apps/details?id=%0D%0Dसंपर्क करें 902-412-3007%0DGmail: primeapp.co.in@gmail.com%0D%0Dवीडियो खबरों के लिए नीचे दिए लिंक को पर क्लिक करें और तुरंत सब्सक्राइब करें ।%0D',
            url: 'http://primeapp.co.in/each-news/'+ news_id +''
        }
...
function getMoreCategoryNews() {
    $.ajax({
        url:"/mobile/getMoreCategoryNews/" + category_name + "/" +offset,
        dataType: "jsonp",
        type:"GET",
        success: function(news){
            // console.log(news);
...
function getVideos() {
    $.ajax({
        ...
        success: function(news){
            // console.log(news);
            for(var i=0;i<news.length;i++){
                var news_card = createVideoCard(news[i]);
                $('#bottom-section').append(news_card);
            }
        },
        error: function(err){
...
function getMoreProfileNews(user_id) {
    $.ajax({
        ...
        success: function(news){
            // console.log(news);
            for(var i=0;i<news.length;i++){
                var news_card = createProfileCard(news[i]);
                $('#bottom-section').append(news_card);
            }
        },
        error: function(err){
